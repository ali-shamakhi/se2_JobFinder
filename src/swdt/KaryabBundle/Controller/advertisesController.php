...
use swdt\KaryabBundle\Form\advertisesFormType;
...
class advertisesController extends Controller
{
    ...
    {
        $repository = $this->getDoctrine()->getRepository('swdtKaryabBundle:Advertise');

        $advertises_list = $repository->findAll();

        return $this->render('@swdtKaryab/panel_karjoo/list_ads.html.twig', array('advertises_list' => $advertises_list));
    }
    ...
    public function relevantAction()
    {
        ...
        $sql = "SELECT advertise_id as ad FROM swdt_advertise_tags";
        ...
        $dql="SELECT ad FROM swdtKaryabBundle:Advertise ad";
       ...
       // $dql.=" AND ad.grade>=".$this->getUser()->getKarjoo()->getGrade();
        ...
        return $this->render('@swdtKaryab/panel_karjoo/list_ads.html.twig', array(
            'ads' => $advertises_list,
            'total'=>$totalCount,
    ...
    private function notifReleventUsers(Advertise $advertise)
    {
        ...
        $sql = "SELECT user_id FROM swdt_users_skills WHERE user_id is not NULL";
        if(count($userSkills)>0)
        ...
        $sql="SELECT email FROM swdt_users";
    ...
    public function newAction(Request $request)
    {
        ...
        return $this->render('@swdtKaryab/panel_karfarma/advertise/new.html.twig',array(
    ...
    public function listKarfaramaAction(){
        ...
        $ads = $this->getDoctrine()->getRepository('swdtKaryabBundle:Advertise')
            ->findBy(array(
                    'user'=>$this->getUser(),
                    'isDeleted'=>false,
        ...
        return  $this->render('@swdtKaryab/panel_karfarma/advertise/list.html.twig',array(
           'ads'=>$ads,
            'total' => count($ads),
        ));
